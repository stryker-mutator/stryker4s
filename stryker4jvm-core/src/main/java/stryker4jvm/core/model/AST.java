...
/**
 * Base class for a tree-like structure. It is required to override hashCode and equals in order for
 * the object to be usable in maps. This is enforced because not overriding these may lead to bugs
 * that are hard to debug. Ideally if the tree structure in the programming language you are using
 * were of type 'scala.meta.Tree' bound to the variable 'tree', then 2 separate instances of AST
 * with this tree as field should be considered equal to each other. Note that this implicitly means
 * that you should not modify the internal structure of an AST instance once it is initialized as
 * that would possibly lead to an incorrect contract between hashCode and equals. Instead, it is
 * highly suggested that you modify a copy of the AST.
 */
public abstract class AST {
  /**
   * Creates a string from an AST which can be parsed by an associated {@link Parser} to produce an
   * equal instance of AST internally. Note that this does NOT necessarily mean that some AST 'a'
   * should lead to a.equals(parse(a.syntax())).
   */
  public abstract String syntax();

  /**
   * Inherited from {@link Object#hashCode()}: Returns a hash code value for the object. This method
   * is supported for the benefit of hash tables such as those provided by {@link
   * java.util.HashMap}.
   *
   ...
   *   <li>If two objects are equal according to the {@link Object#equals(Object) equals} method,
   *       then calling the {@code hashCode} method on each of the two objects must produce the same
   *       integer result.
   *   <li>It is <em>not</em> required that if two objects are unequal according to the {@link
   *       Object#equals(Object) equals} method, then calling the {@code hashCode} method on each of
   *       the two objects must produce distinct integer results. However, the programmer should be
  ...
  /**
   * Inherited from {@link Object#equals(Object)}: Indicates whether some other object is "equal to"
   * this one.
   *
   ...
   * @apiNote It is generally necessary to override the {@link Object#hashCode hashCode} method
   *     whenever this method is overridden, so as to maintain the general contract for the {@code
