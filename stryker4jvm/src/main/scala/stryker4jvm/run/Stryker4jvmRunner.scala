...
import stryker4jvm.extensions.Stryker4jvmCoreConversions._
...
abstract class Stryker4jvmRunner(implicit log: Logger) {
  def run(): IO[ScoreStatus] = {
    ...
    SupportedLanguageMutators.languageRouter.values.foreach(mutator => mutator.setLanguageConfig(config.asLanguageMutatorConfig))
