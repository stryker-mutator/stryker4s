...
//# sourceMappingURL=lit-html.js.map
  ...
  \****************************************************/
    ...
    `;
            ...
            renderTitle() {
                if (this.context) {
                    return lit_element_1.html `<h1 class="display-4">${this.name}</h1>`;
                }
                else {
                    return undefined;
                }
        ...
        `;
            ...
            renderMutationTestReport() {
                if (this.context && this.report) {
                    return lit_element_1.html `<mutation-test-report-file .name="${this.name}" .thresholds="${this.report.thresholds}" .model="${this.context}"></mutation-test-report-file>`;
                }
                else if (this.report) {
    ...
    `
        ...
        __decorate([
            lit_element_1.property()
        ], MutationTestReportAppComponent.prototype, "name", void 0);
            ...
            annotatedCode() {
                ...
                const annotateCharacter = (char, line, column) => {
                    ...
                    if (mutantsStarting.length || mutantsEnding.length) {
                        ...
                        const backgroundColorStartElement = `<span class="bg-${backgroundState.determineBackground()}">`;
                        const backgroundColorEndElement = '</span>';
                        const mutantComponentStartElements = mutantsStarting.map(m => `<mutation-test-report-mutant mutantId="${m.id}" mutatorName="${m.mutatorName}" status="${m.status}"><span slot="replacement">${m.replacement}</span><span slot="actual">`);
                        const mutantComponentsEndElements = mutantsEnding.map(() => `</span></mutation-test-report-mutant>`);
                        return `${backgroundColorEndElement}${mutantComponentsEndElements.join('')}${mutantComponentStartElements.join('')}${backgroundColorStartElement}${helpers_1.escapeHtml(char)}`;
                    }
                    else {
                        return helpers_1.escapeHtml(char);
                    }
                };
                return `<span>${walkCode(this.model.source, annotateCharacter)}</span>`;
            }
        };
    ...
    /***/ (function(module, exports, __webpack_require__) {
        ...
        function normalizeFileNames(files) {
            ...
            fileNames.forEach(fileName => {
                result[fileName.substr(commonBasePath.length)] = files[fileName];
            });
            return result;
        }
