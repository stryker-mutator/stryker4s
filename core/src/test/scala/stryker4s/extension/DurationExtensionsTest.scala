...
class DurationExtensionsTest extends Stryker4sSuite {
  describe("toHumanReadable") {
    ...
    it("should not include units in the middle that are zero") {
      (3.hours + 0.minutes + 10.seconds).toHumanReadable shouldBe "3 hours, 10 seconds"
    }

    it("should parse a combination of units") {
      (2.days + 3.hours + 2.minutes + 5.seconds).toHumanReadable shouldBe "2 days, 3 hours, 2 minutes, 5 seconds"
    }
  }
}
