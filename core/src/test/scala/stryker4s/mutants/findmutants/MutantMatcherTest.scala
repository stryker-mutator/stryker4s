...
class MutantMatcherTest extends Stryker4sSuite {
  ...
  def expectMutations(
      matchFun: PartialFunction[Tree, Seq[Either[IgnoredMutations, Mutations]]],
      tree: Tree,
      original: Term,
      expectedTerms: Term*
  ...
  def expectMutations(
    ...
    expectedMutations.foreach { expectedMutation =>
      val actualMutant = actualMutants
        ...
        .flatten
        .find(_.mutatedStatement.isEqual(expectedMutation))
        .getOrElse(fail(s"mutant $expectedMutation not found"))

      actualMutant.metadata.original shouldBe original.syntax
      assert(actualMutant.mutatedStatement.isEqual(expectedMutation), actualMutant.mutatedStatement)
      actualMutant.metadata.mutatorName shouldBe expectedName
    }
  }
  ...
  describe("All Matchers") {
    it("should match a conditional statement") {
      ...
      found.collect { case Right(v) => v }.flatMap(_.toVector) should have length 7
    ...
    it("should match the default case of a constructor argument") {
      ...
      val found = tree.collect(sut.allMatchers).map(_(PlaceableTree(tree.ctor.paramss.head.head)))
  ...
  describe("no function name matching") {
    ...
    //   val result = tree.collect(sut.allMatchers).map(_(PlaceableTree(tree.body))).collect { case Right(v) => v }
    ...
    //   val trueFound = trueTree.collect(sut.allMatchers).map(_(PlaceableTree(tree.body)))
    //   val falseFound = falseTree.collect(sut.allMatchers).map(_(PlaceableTree(tree.body)))
