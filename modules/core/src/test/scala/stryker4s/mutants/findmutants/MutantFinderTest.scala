...
class MutantFinderTest extends Stryker4sIOSuite with LogMatchers {
  ...
  test("parseFile should parse an existing file") {
  ...
  test("parseFile should throw an exception on a non-parseable file") {
    val sut = new MutantFinder()
  ...
  test("parseFile should fail on a nonexistent file") {
    val sut = new MutantFinder()
  ...
  test("parseFile should parse a scala-3 file") {
  ...
  test("logging should error log an unfound file") {
    implicit val config: Config = Config.default
