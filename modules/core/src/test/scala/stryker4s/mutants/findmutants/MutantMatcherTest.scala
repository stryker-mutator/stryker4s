...
class MutantMatcherTest extends Stryker4sSuite {
  ...
  def expectMutations(
      ...
      original: Term,
      expectedTerms: Term*
  )(expectedName: String)(implicit loc: Location): Unit = {
    ...
    expectedTerms.foreach(expectedTerm => expectMutations(found, original, expectedTerm)(expectedName))
  }
  ...
  test("All Matchers should match a conditional statement") {
    ...
    expectMutations(found, Term.Name("&&"), Term.Name("||"))("LogicalOperator")
    expectMutations(found, Term.Name("<"), Term.Name("<="), Term.Name(">"), Term.Name("=="))("EqualityOperator")
  }

  test("All Matchers should match a method") {
    val mutatorName = "MethodExpression"
  ...
  test("All Matchers should match a boolean and a conditional") {
    val tree = "def foo = false && 15 > 4".parseDef

    val found = tree.collect(sut.allMatchers).map(_(PlaceableTree(tree.body)))

    assertEquals(found.flatMap(_.toSeq).flatMap(_.toVector).length, 5)
    expectMutations(found, Lit.Boolean(false), Lit.Boolean(true))("BooleanLiteral")
    expectMutations(found, Term.Name("&&"), Term.Name("||"))("LogicalOperator")
    expectMutations(found, Term.Name(">"), Term.Name("<"), Term.Name("=="))("EqualityOperator")
  }

  test("All Matchers should match the default case of a constructor argument") {
  ...
  test("All Matchers should match on the default case of a function argument") {
  ...
  test("All Matchers original should have correct location and replacement") {
    val tree = """def foo = {
                      baz match {
                        case t =>
                          // comment
  ...
  val equalityMutatorName = "EqualityOperator"
  test("matchEqualityOperator matcher should match >= sign with >, <, and ==") {
  ...
  test("matchEqualityOperator matcher should match > with >=, < and ==") {
    val tree = "def foo = 18 > 20".parseDef
    val found = tree.collect(sut.matchEqualityOperator).map(_(PlaceableTree(tree.body)))
    expectMutations(
      found,
  ...
  test("matchEqualityOperator matcher should match <= to <, >= and ==") {
    val tree = "def foo = 18 <= 20".parseDef
    val found = tree.collect(sut.matchEqualityOperator).map(_(PlaceableTree(tree.body)))
    expectMutations(
      found,
  ...
  test("matchEqualityOperator matcher should match == to !=") {
    val tree = "def foo = 18 == 20".parseDef
    val found = tree.collect(sut.matchEqualityOperator).map(_(PlaceableTree(tree.body)))
    expectMutations(
      found,
  ...
  val logicalMutatorName = "LogicalOperator"

  test("matchLogicalOperator matcher should match && to ||") {
    expectMutations(
  ...
  test("matchBooleanLiteral matcher should match true to false") {
    expectMutations(
      sut.matchBooleanLiteral,
  ...
  val stringMutatorName = "StringLiteral"

  test("matchStringLiteral matcher should match foo to NonEmptyString") {
    expectMutations(
  ...
  test("matchStringLiteral matcher should match empty string to StrykerWasHere") {
    expectMutations(
      sut.matchStringLiteral,
      """def foo = "" """.parseDef,
      EmptyString.tree,
  ...
  test("matchStringLiteral matcher should match on interpolated strings") {
    val interpolated =
  ...
  test("matchStringLiteral matcher should match once on interpolated strings with multiple parts") {
  ...
  test("matchStringLiteral matcher should not match non-string interpolation") {
    val interpolated =
  ...
  test("matchStringLiteral matcher should not match pattern interpolation") {
    val tree = """class Foo {
        def bar = {
  ...
  test("matchStringLiteral matcher should match pattern in string") {
    val tree = """def bar = {
          case "str" => 4
        }""".parseDef
    expectMutations(
  ...
  test("matchStringLiteral matcher should not match xml literals") {
  ...
  test("matchStringLiteral matcher should not match empty strings on xml literals") {
    val tree = s"""class Foo {
        def bar = <foo>{foo}</foo>
      }""".parseSource
  ...
  test("matchStringLiteral matcher should match inside xml literal args") {
  ...
  test("matchStringLiteral matcher should not match xml interpolation") {
  ...
  val regexMutatorName = "RegularExpression"
  val regex = Lit.String(".*")

  test("regexMutator should match Regex constructor") {
  ...
  test("regexMutator should match scala.util.matching.Regex constructor") {
    expectMutations(
      sut.matchRegex,
  ...
  test("regexMutator should match a Regex constructor with named groups") {
    expectMutations(
      sut.matchRegex,
  ...
  test("regexMutator should match Regex String ops") {
    expectMutations(
      sut.matchRegex,
      """def foo = ".*".r""".parseDef,
      regex,
  ...
  test("regexMutator should match java.util.regex.Pattern.compile Regex constructor") {
    expectMutations(
      sut.matchRegex,
  ...
  test("regexMutator should match Pattern.compile Regex constructor with flags") {
    expectMutations(
      sut.matchRegex,
  ...
  test("regexMutator should not match e regular string") {
    expectMutations(
      sut.matchRegex,
      """def foo = ".*"""".parseDef,
      regex
  ...
  test("regexMutator should handle regexes without any mutations") {
  ...
  test("no function name matching should not match a function with a mutator name") {
  ...
  test("no function name matching should not match on a case class with a mutator name") {
  ...
  test("no function name matching should not match on a variable with a mutator name") {
  ...
  test("no function name matching should not match on type arguments") {
  ...
  test("no function name matching should not match on infix type arguments") {
  ...
  test("no function name matching should not match on type apply") {
  ...
  test("no function name matching should not match on literal type declarations") {
  ...
  test("no function name matching should not match on literal type declarations for var") {
  ...
  test("no function name matching should not match on infix literal type declarations") {
  ...
  test("no function name matching should not match on def literal return types") {
  ...
  test("no function name matching should not match on literal function types") {
  ...
  test("no function name matching should not match on type aliases") {
  ...
  test("no function name matching should not match a function with a single expression") {
  ...
  test("no function name matching should mutate if statements with true and false as condition") {
    val mutatorName = "ConditionalExpression"
    ...
    expectMutations(found, Term.Name("aVariable"), Lit.Boolean(true))(mutatorName)
    expectMutations(found, Term.Name("aVariable"), Lit.Boolean(false))(mutatorName)
  }

  test("no function name matching should mutate while statements with false as condition") {
    val mutatorName = "ConditionalExpression"
  ...
  test("no function name matching should mutate do while statements with false as condition") {
    val mutatorName = "ConditionalExpression"
  ...
  test(
    "no function name matching should mutate conditional statements that have a literal boolean as condition only once"
  ) {
    val mutatorName = "BooleanLiteral"
    ...
    expectMutations(trueFound, Lit.Boolean(true), Lit.Boolean(false))(mutatorName)
    expectMutations(falseFound, Lit.Boolean(false), Lit.Boolean(true))(mutatorName)
  }

  import cats.syntax.all.*
  test("filtering should filter out config excluded mutants") {
    implicit val conf: Config = Config.default.copy(excludedMutations = Seq(ExcludedMutation("LogicalOperator")))
    val sut = new MutantMatcherImpl()(using conf)
    val tree = "def foo = 15 > 20 && 20 < 15".parseDef
  ...
  test("filtering should filter out string mutants inside annotations") {
  ...
  test("filtering should log partition unparsable regular expressions") {
