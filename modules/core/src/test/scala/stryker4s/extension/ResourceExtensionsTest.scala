...
class ResourceExtensionsTest extends Stryker4sIOSuite {
  test("selfRecreatingResource should create a resource only once when not using the release F") {
    val op = for {
  ...
  test("selfRecreatingResource should close first before creating a new Resource") {
    val op = for {
      log <- Ref[IO].of(List.empty[String])
      _ <- logged(log)
