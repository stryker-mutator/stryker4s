...
class FileExtensionsTest extends Stryker4sSuite {
  ...
  test("relativePath should return the relative path on a file inside the base-dir") {
  ...
  test("relativePath should return just the file name when a file is in the base-dir") {
  ...
  test("relativePath should return the path if the given path is relative") {
    val result = Path("src/main").relativePath

    assertEquals(result, Path("src/main"))
  }

  test("relativePath works if both are relative") {
    val result = (Path("module") / "src" / "main").relativePath(Path("module"))
    assertEquals(result, Path("src") / "main")
  }

  test("relativePath works if both are absolute") {
    val result =
      (config.baseDir / "module" / "src" / "main").relativePath(config.baseDir / "module")
    assertEquals(result, Path("src/main"))
  }

  test("relativePath works if baseDir is relative and path is absolute") {
    val result = (absoluteBaseDir / "module" / "src" / "main").relativePath(Path("module"))
    assertEquals(result, Path("src/main"))
  }

  test("relativePath works if baseDir is absolute and path is relative") {
    val result = Path("module/src/main").relativePath(absoluteBaseDir / "module")
    assertEquals(result, Path("src/main"))
  }

  test("relativePath multiple calls doesn't change the path") {
    val result = Path("src/main").relativePath.relativePath.relativePath
    assertEquals(result, Path("src/main"))
  }

  test("inSubDir should calculate a path relative to the new subDir") {
