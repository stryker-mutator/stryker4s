...
class GlobFileResolverTest extends Stryker4sIOSuite {
  ...
  test("files should not collect the baseDir") {
  ...
  test("files should find all scala files and not the non-scala files with a default glob") {
  ...
  test("files should find matching files with custom match pattern") {
  ...
  test("files should only add a glob once even when it matches twice") {
  ...
  test("files should exclude all files in the target folder") {
  ...
  test("files should match if basePath and glob are absolute") {
    val sut =
