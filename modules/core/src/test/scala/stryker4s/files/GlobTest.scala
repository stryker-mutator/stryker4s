...
class GlobTest extends Stryker4sSuite {
  ...
  test("include patterns should match no files with empty glob") {
    val matcher = Glob.matcher(path, Seq.empty)

    assert(!matcher.matches(path / "foo.scala"))
  }

  test("include patterns should match files 0 levels deep") {
    val matcher = Glob.matcher(path, Seq("*.scala"))

    assert(matcher.matches(path / "foo.scala"))
  }

  test("include patterns should match on the second glob if the first doesn't match") {
    val matcher = Glob.matcher(path, Seq("*.sbt", "*.scala"))

    assert(matcher.matches(path / "foo.scala"))
  }

  test("include patterns should match files 1 level deep") {
  ...
  test("include patterns should match files multiple levels deep") {
    val matcher = Glob.matcher(path, Seq("src/**/*.scala"))
  ...
  test("include patterns should match on multiple patterns") {
  ...
  test("include patterns should match exact matches") {
    val matcher = Glob.matcher(path, Seq("src/main/scala/someFile.scala"))

    assert(matcher.matches(path / "src" / "main" / "scala" / "someFile.scala"))
  }

  test("ignore patterns should exclude the file specified in the excluded pattern") {
  ...
  test("ignore patterns should exclude all files specified in the excluded pattern") {
  ...
  test("ignore patterns should exclude all files based on a wildcard") {
    val matcher = Glob.matcher(
      path,
  ...
  test("ignore patterns should not exclude files if a non-matching ignore pattern is given") {
    val matcher = Glob.matcher(
      path,
