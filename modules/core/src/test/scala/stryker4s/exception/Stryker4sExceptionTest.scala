...
class Stryker4sExceptionTest extends Stryker4sSuite {
  test("UnableToBuildPatternMatchException should have the correct message") {
  ...
  test("InitialTestRunFailedException should have the correct message") {
    assertNoDiff(InitialTestRunFailedException("testMsg").getMessage, "testMsg")
  }

  test("TestSetupException should have the correct message ") {
    assertNoDiff(
  ...
  test("TestSetupFailedException should use the message") {
    assertNoDiff(
  ...
  test("MutationRunFailedException should have the correct message") {
    assertNoDiff(MutationRunFailedException("xyz").getMessage, "xyz")
  }

  test("UnableToFixCompilerErrorsException should have a nicely formatted message") {
