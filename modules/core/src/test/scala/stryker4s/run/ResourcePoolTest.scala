...
class ResourcePoolTest extends Stryker4sIOSuite {
  ...
  test("resource should close the resources when closing the pool") {
  ...
  test("run should divide work on the resource pool") {
  ...
  test("run should divide work over other resources if one is slower") {
  ...
  test("loan should put the resource back in the pool after use") {
