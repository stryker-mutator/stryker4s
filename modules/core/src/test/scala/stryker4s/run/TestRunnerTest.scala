...
class TestRunnerTest extends Stryker4sIOSuite with LogMatchers with TestData {
  ...
  test("timeoutRunner initialTestRun should complete the timeout after the initial testrun") {
    val op = for {
  ...
  test("timeoutRunner initialTestRun should use the reported timeout if available") {
  ...
  test("timeoutRunner initialTestRun should not log the timeout setting if timeout has already been completed") {
    val op = for {
      timeout <- Deferred[IO, FiniteDuration]
  ...
  test("timeoutRunner runMutant should timeout slow mutant runs") {
  ...
  test("timeoutRunner runMutant should recreate the inner resource after a timeout") {
    val op = for {
      timeout <- Deferred[IO, FiniteDuration]
      // Set timeout to something short
      _ <- timeout.complete(1.millisecond)
  ...
  test("timeoutRunner runMutant should not timeout fast mutant runs") {
    val mutant = createMutant
    val op = for {
      timeout <- Deferred[IO, FiniteDuration]
      // Set timeout to something short
  ...
  test("retryRunner should retry after a first attempt fails") {
    val mutant = createMutant
  ...
  test("retryRunner should report a mutant as Error after 3 failures") {
    val mutant = createMutant
    val op = for {
      log <- Ref[IO].of(List.empty[String])
  ...
  test("retryRunner should only report as Error after 3 failed attempts") {
    val mutant = createMutant
  ...
  test("maxReuseTestRunner should not recreate if reuse is not reached yet") {
  ...
  test("maxReuseTestRunner should recreate when reuse is reached") {
